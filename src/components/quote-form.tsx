  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        equipment: (parseJsonOrNull(equipmentRaw) ?? []) as never,
        highlights: (parseJsonOrNull(highlightsRaw) ?? { included: [], excluded: [] }) as never,
        itinerary: (parseJsonOrNull(itineraryRaw) ?? []) as never,
        period_start: periodStart || null,
        period_end: periodEnd || null,
        party_size: partySize ? Number(partySize) : null,
        currency,
