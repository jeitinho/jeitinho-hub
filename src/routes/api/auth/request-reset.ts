...
import { hashPassword, getSessionToken } from "@/lib/auth/cloudflare-auth";
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        if (!env.RESEND_API_KEY || !env.RESEND_FROM) {
          return Response.json({ ok: false, error: "La réinitialisation par email n'est pas encore configurée." }, { status: 503 });
        }
        ...
        await env.DB.prepare("INSERT INTO auth_password_resets (id,user_id,token_hash,expires_at,created_at) VALUES (?,?,?,?,?)")
          .bind(crypto.randomUUID(), user.id, tokenHash, expires, now).run();
        ...
        const response = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            to: [user.email],
            subject: "Réinitialiser votre mot de passe — JEITINHO Hub",
            html: `<p>Vous avez demandé à réinitialiser votre mot de passe JEITINHO Hub.</p><p><a href="${resetUrl}">Réinitialiser mon mot de passe</a></p><p>Ce lien expire dans 30 minutes.</p>`,
          }),
        });
        if (!response.ok) {
