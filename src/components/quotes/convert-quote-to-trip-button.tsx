...
export function ConvertQuoteToTripButton({ quoteId }: { quoteId: string }) {
  ...
  const {data,isLoading}=useQuery({queryKey:["quote-trip-link",quoteId],queryFn:async()=>{const [{data:quote,error:quoteError},{data:trip,error:tripError}]=await Promise.all([supabase.from("quotes").select("id,status,number,reference").eq("id",quoteId).single(),supabase.from("trips").select("id,reference,status").eq("quote_id",quoteId).maybeSingle()]);if(quoteError)throw new Error(quoteError.message);if(tripError)throw new Error(tripError.message);return {quote:quote as any,trip:trip as any}}});
