...
class QueryBuilder<T = Record<string, unknown>> implements PromiseLike<DbResult<T | T[]>> {
  ...
  constructor(table: string) {
    this.payload = { operation: "select", table, columns: "*", filters: [], orders: [] };
  }

  select(columns = "*", _options?: { count?: "exact" | "planned" | "estimated"; head?: boolean }) {
    if (this.payload.operation === "select") {
  ...
  or(expression: string) {
    // Preserve the common PostgREST form: column.eq.value,column.ilike.%value%.
    for (const part of expression.split(",")) {
      const match = part.match(/^([a-zA-Z_][a-zA-Z0-9_]*)\.(eq|neq|gt|gte|lt|lte|like|ilike|is)\.(.*)$/);
      if (!match) continue;
      this.payload.filters.push({ column: match[1], operator: match[2] as Filter["operator"], value: decodeURIComponent(match[3]) });
    }
    return this;
  }

  order(column: string, options?: { ascending?: boolean; nullsFirst?: boolean }) {
    this.payload.orders.push({ column, ascending: options?.ascending !== false, nullsFirst: options?.nullsFirst });
    return this;
  }
  ...
  private filter(operator: Filter["operator"], column: string, value: unknown) {
  ...
  private async execute(): Promise<DbResult<T | T[]>> {
    try {
      const response = await fetch("/api/db/query", {
        ...
        body: JSON.stringify({ ...this.payload, returning: this.returning, singleMode: this.singleMode }),
      });
  ...
  from(bucket: string) {
    return {
      upload: async (path: string, file: File, options?: { upsert?: boolean; contentType?: string }) => {
        ...
        form.set("file", file);
        const response = await fetch("/api/storage/upload", { method: "POST", credentials: "include", body: form });
      ...
      createSignedUrl: async (path: string, expiresIn: number) => {
        ...
        const response = await fetch(`/api/storage/signed-url?${params.toString()}`, { credentials: "include" });
        const body = await response.json().catch(() => null);
...
class DatabaseClient {
  ...
  async rpc<T = unknown>(fn: string, args: Record<string, unknown> = {}): Promise<DbResult<T>> {
    try {
      const response = await fetch("/api/db/rpc", { method: "POST", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ fn, args }) });
      const body = (await response.json().catch(() => null)) as DbResult<T> | null;
      if (!response.ok) return { data: null, error: body?.error ?? { message: `RPC failed (${response.status})` } };
      return body ?? { data: null, error: { message: "Empty RPC response" } };
    } catch (error) {
