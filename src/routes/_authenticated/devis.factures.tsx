...
function InvoicesList() {
  ...
  const newInvoiceLink = (
    <Link to="/devis/factures" search={{ new: "1" }} className="btn-primary inline-flex h-9 items-center rounded-md px-4 text-sm">
    ...
    <PageShell
      ...
      actions={
        <div className="flex gap-2">
          <Link to="/devis">
            <button className="rounded-md border border-border/60 px-3 py-2 text-xs text-muted-foreground hover:text-foreground">
              Devis
            </button>
          </Link>
          {newInvoiceLink}
    ...
    >
      ...
      {isLoading ? (
        ...
        <Card className="border-destructive/40 p-8">
          <h2 className="font-semibold">Impossible de charger les factures</h2>
        ...
        <Card className="border-dashed p-16 text-center">
          ...
          <div className="mt-6 inline-flex">{newInvoiceLink}</div>
        </Card>
      ) : (
