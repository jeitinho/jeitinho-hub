...
export async function fetchPriorityLeads() {
  const { data, error } = await supabase.from("leads").select(LEAD_SELECT).in("status", OPEN_STATUSES).order("score", { ascending: false }).order("received_at", { ascending: false });
...
export async function rescoreLead(lead: PriorityLead) {
  ...
  const { error } = await supabase.from("leads").update({ score: scored.score, priority: scored.priority, score_breakdown: scored.breakdown, estimated_value: lead.estimated_value ?? estimateValue({ ...lead, highSeason }) }).eq("id", lead.id);
  if (error) throw new Error(error.message);
  return scored;
}
...
export async function markLeadContacted(lead: PriorityLead) {
  ...
  const { error } = await supabase.from("leads").update({ status: lead.status === "new" ? "contacted" : lead.status, pipeline_stage: "contacte", last_contact_at: now.toISOString(), next_action: "Envoyer la proposition", next_action_at: new Date(now.getTime() + delay * 3_600_000).toISOString() } as never).eq("id", lead.id);
  if (error) throw new Error(error.message);
}
export async function setLeadNextAction(id: string, action: string, atIso: string | null) {
  const { error } = await supabase.from("leads").update({ next_action: action || null, next_action_at: atIso }).eq("id", id);
  if (error) throw new Error(error.message);
}
...
export async function fetchSentQuotes() {
  const [{ data, error }, { data: clients, error: clientError }] = await Promise.all([
    supabase.from("quotes").select(QUOTE_FOLLOWUP_SELECT).eq("status", "sent").order("updated_at", { ascending: false }),
...
export async function fetchOpenTasks() {
  const { data, error } = await supabase.from("crm_tasks").select("id,kind,channel,status,stage,title,message_draft,due_at,quote_id,lead_id").in("status", ["a_valider", "valide"]).order("due_at");
  if (error) throw new Error(error.message);
...
export async function generateDueFollowups(quotes: FollowupQuote[], existing: CrmTask[]) {
  ...
  const { error } = await supabase.from("crm_tasks").upsert(rows as never, { onConflict: "quote_id,stage" });
  if (error) throw new Error(error.message);
  return rows.length;
}
...
export async function markTaskSent(task: CrmTask) {
  ...
  const { error } = await supabase.from("crm_tasks").update({ status: "envoye", handled_at: nowIso }).eq("id", task.id);
  if (error) throw new Error(error.message);
  if (task.quote_id && task.stage) {
    const { error: quoteError } = await supabase.from("quotes").update({ followup_stage: task.stage, last_contact_at: nowIso, next_action: task.stage < FOLLOWUP_LABELS.length ? FOLLOWUP_LABELS[task.stage] : "Clôturer le dossier" }).eq("id", task.quote_id);
    if (quoteError) throw new Error(quoteError.message);
  }
}
export async function cancelTask(id: string) { const { error } = await supabase.from("crm_tasks").update({ status: "annule", handled_at: new Date().toISOString() }).eq("id", id); if (error) throw new Error(error.message); }
export async function snoozeTask(id: string, days = 2) { const { error } = await supabase.from("crm_tasks").update({ due_at: new Date(Date.now() + days * 86_400_000).toISOString() }).eq("id", id); if (error) throw new Error(error.message); }
export async function toggleQuoteFollowups(id: string, paused: boolean) { const { error } = await supabase.from("quotes").update({ followup_paused: paused }).eq("id", id); if (error) throw new Error(error.message); }
