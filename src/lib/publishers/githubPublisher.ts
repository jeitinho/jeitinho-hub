...
export const githubPublisher: Publisher = {
  ...
  async publish(ctx: PublisherContext): Promise<PublisherResult> {
    const { data: content, error } = await supabase.from("contents").select("id,slug,title,subtitle,excerpt,body_json,tags,published_at,reading_time_min,seo_title,seo_description,language,metadata,author_id,category_id,cover_media_id,type").eq("id", ctx.contentId).single();
    ...
    const [authorRes, categoryRes, coverRes] = await Promise.all([
      c.author_id ? supabase.from("authors").select("slug").eq("id", c.author_id).maybeSingle() : Promise.resolve({ data: null, error: null }),
      c.category_id ? supabase.from("content_categories").select("slug").eq("id", c.category_id).maybeSingle() : Promise.resolve({ data: null, error: null }),
      c.cover_media_id ? supabase.from("media").select("url").eq("id", c.cover_media_id).maybeSingle() : Promise.resolve({ data: null, error: null }),
    ]);
    ...
    if (ctx.dryRun) {
      if ((channel as any)?.id) await supabase.from("publications").insert({ channel_id: (channel as any).id, content_id: c.id, status: "pending", payload, published_by: ctx.triggeredBy, scheduled_at: null });
    ...
    try {
      ...
      if ((channel as any)?.id) await supabase.from("publications").insert({ channel_id: (channel as any).id, content_id: c.id, status: "failed", payload: { ...payload, error: msg }, published_by: ctx.triggeredBy });
      return { ok: false, error: `Push GitHub échoué: ${msg}` };
    }
    if (!pushRes.ok) {
      if ((channel as any)?.id) await supabase.from("publications").insert({ channel_id: (channel as any).id, content_id: c.id, status: "failed", payload: { ...payload, error: pushRes.error, conflict: !!pushRes.conflict }, published_by: ctx.triggeredBy });
    ...
    await supabase.from("contents").update({ status: "published", published_at: publishedAt }).eq("id", c.id);
    ...
    if ((channel as any)?.id) await supabase.from("publications").insert({ channel_id: (channel as any).id, content_id: c.id, status: "success", payload: enriched, external_ref: pushRes.commitSha ?? targetPath, external_url: pushRes.commitUrl ?? null, published_by: ctx.triggeredBy, published_at: publishedAt });
