...
function Ticketing() {
  const { data, isLoading, error } = useQuery({ queryKey: ["ticket-offers"], queryFn: async () => { const { data, error } = await (supabase as any).from("ticket_offers").select("*").order("event_date", { ascending: true }); if (error) throw error; return (data ?? []) as any[]; } });
  const rows = data ?? [];
